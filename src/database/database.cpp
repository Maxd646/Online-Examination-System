...
bool DatabaseManager::authenticateUser(const std::string& username, const std::string& password, User& user) {
...
std::vector<Question> DatabaseManager::getRandomQuestions(int count, const std::string& subject) {
    std::string sql = R"(
        SELECT id, subject, question_text, option1, option2, option3, option4, 
               correct_answer, difficulty, explanation, created_at, updated_at, created_by, is_active
...
std::vector<ExamResult> DatabaseManager::getExamResultsByUser(int userId) {
    ...
    if (stmt) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            if (startTime) result.setStartTime(startTime);
            if (endTime) result.setEndTime(endTime);
            if (subject) result.setSubject(subject);
            if (difficulty) result.setDifficulty(difficulty);
...
Question::Question(int id, const std::string& subject, const std::string& questionText,
                  const std::vector<std::string>& options, int correctAnswer,
                  const std::string& difficulty, const std::string& explanation)
...
bool Question::isValid() const {
    return !subject.empty() && !questionText.empty() && 
           ...
           std::all_of(options.begin(), options.end(), [](const std::string& opt) {
               return !opt.empty();
           });
}

void Question::display() const {
    std::cout << "\nQ" << id << ". " << questionText << std::endl;
    std::cout << "Subject: " << subject << " | Difficulty: " << difficulty << std::endl;
...
void Question::displayWithAnswer() const {
    display();
    std::cout << "Correct Answer: " << (correctAnswer + 1) << ". " << options[correctAnswer] << std::endl;
...
ExamResult::ExamResult(int userId, const std::string& username, int score, int totalQuestions,
                      const std::string& subject, const std::string& difficulty)
...
std::string ExamResult::getGrade() const {
...
void ExamResult::display() const {
    std::cout << "Exam Result ID: " << id << std::endl;
    std::cout << "Student: " << username << std::endl;
    std::cout << "Score: " << score << "/" << totalQuestions << std::endl;
    std::cout << "Percentage: " << std::fixed << std::setprecision(2) << percentage << "%" << std::endl;
    std::cout << "Grade: " << getGrade() << std::endl;
    std::cout << "Date: " << examDate << std::endl;
    std::cout << "Status: " << (isPassed() ? "PASSED" : "FAILED") << std::endl;
}
...
int DatabaseManager::getTotalUsers() {
    ...
    if (stmt) finalizeStatement(stmt);
    return 0;
}
...
std::vector<Question> DatabaseManager::getQuestionsBySubject(const std::string& subject) {
    const char* sql = R"(
        SELECT id, subject, question_text, option1, option2, option3, option4, 
               correct_answer, difficulty, explanation, is_active 
...
std::vector<Question> DatabaseManager::getQuestionsByDifficulty(const std::string& difficulty) {
    const char* sql = R"(
        SELECT id, subject, question_text, option1, option2, option3, option4, 
               correct_answer, difficulty, explanation, is_active 
...
std::vector<Question> DatabaseManager::searchQuestions(const std::string& keyword) {
    const char* sql = R"(
        SELECT id, subject, question_text, option1, option2, option3, option4, 
               correct_answer, difficulty, explanation, is_active 
        FROM questions 
...
std::vector<ExamResult> DatabaseManager::getAllExamResults() {
    const char* sql = R"(
    ...
    if (stmt) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            if (examDate) result.setExamDate(examDate);
